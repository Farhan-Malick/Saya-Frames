	...
	class Dbase{
      ...
      if ($row["Id"] != "") {
		...
		function setDBConnections($db_nsme = "", $db_host = "", $db_user = "", $db_pass = ""){
			...
			// }else {
			// 	$this->USER_NAME = "avvsbgjn_root";
			// }
			// if ($db_pass) {
			// 	$this->USER_PWD = $db_pass;
			// }else {
			// 	$this->USER_PWD = "Slickubl91!";
			// }
			// if ($db_nsme) {
			// 	$this->DATABASE = $db_nsme;
			// }else {
			// 	$this->DATABASE = "avvsbgjn_saya_frames";
			// }
      ...
      if ($this->debug == 1 && $debug) {
		...
		function phastConnection(){
			// $conn = new mysqli($this->MYSQL_HOST, $this->USER_NAME, $this->USER_PWD, $this->DATABASE);
			return $this->write_dbLink;
		}
		...
		function is_url($url){
			if (!($parts = @parse_url($url))) return false;
				...
				if ($parts[scheme] != "http" && $parts[scheme] != "https" && $parts[scheme] != "ftp" && $parts[scheme] != "gopher") return false;
				  ...
				  if (!eregi("^[0-9a-z]([-.]?[0-9a-z])*\.[a-z]{2,3}$", $parts[host], $regs)) return false;
				else
				  if (!eregi("^([0-9a-z-]|[\_])*$", $parts[user], $regs)) return false;
				else
				  if (!eregi("^([0-9a-z-]|[\_])*$", $parts[pass], $regs)) return false;
				else
				  if (!eregi("^[0-9a-z/_\.@~\-]*$", $parts[path], $regs)) return false;
				else
				  if (!eregi("^[0-9a-z?&=#\,]*$", $parts[query], $regs)) return false;
			}
			return true;
		}
